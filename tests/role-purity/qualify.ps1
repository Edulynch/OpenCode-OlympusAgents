...
try {
    ...
    Check RP16_MAINTENANCE_OUTSIDE_ROUTING ($k -match 'Kael → Maintenance remains DENIED' -and $k -match 'cannot be TARGET_ROLE' -and $s -match '(?s)Never directly invoke.*?Maintenance' -and $k -match 'Only veyra, orin, kovan, nox, vera, and sorin are valid child role IDs')
