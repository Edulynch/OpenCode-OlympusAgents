...
- **Orchestrator ownership.** Specialized orchestrators/reasoners may iteratively request or re-consult specialists needed to answer their domain question. They remain coordinators, not owners of exploration, implementation, testing, fixing, or another specialist's execution merely to avoid a handoff.
- **Acyclic orchestration.** Delegation must not form recursive or mutual orchestration loops: an active orchestrator cannot re-enter an ancestor in its current delegation chain (for example, Argus → Thales → Argus, Atlas → Kael → Atlas, or Helios → Atlas → Helios). Focused follow-up with an already consulted specialist is allowed for materially new or narrower evidence, not an active cycle.
...
| 3 — Role Purity + Iterative Evidence | **PLANNED** (3A topology selected) | Live qualification selected Kael-mediated orchestration; implementation and integrated role-purity/iterative-evidence qualification remain ahead. Prove focused follow-up, completion ownership, no cycles or duplicate broad discovery, and bounded routing overhead; establish no-progress rule and diagnostic budget foundations. |
