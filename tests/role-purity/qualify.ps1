...
function New-Loop([bool]$gate = $true) {
    [pscustomobject]@{ Gate=$gate; Thales='thales-original'; Calls=0; Worker=$null;
...
function Consult($loop, [string]$session, [string]$status) {
    if (-not $loop.Gate -or $session -cne $loop.Thales -or $loop.Round -in @('PENDING_OR_INDETERMINATE','COMPLETION_UNCONFIRMED')) { return 'REJECT' }
...
function Route($loop, [string]$key, [string]$role, [string]$path = 'src/probe.py',
    ...
    if (-not $loop.Gate) { return 'NO_THALES' }
...
try {
    $k=Text '.opencode/agents/kael.md'; $s=Text '.opencode/agents/thales.md'
    ...
    Check RP3_THALES_REQUEST ($s -match 'STATUS: EVIDENCE_REQUEST' -and $s -match 'EXPECTED_DISCRIMINATION' -and $k -match 'validate TARGET_ROLE')
    Check RP4_SAME_REASONER ($k -match 'SAME native Thales session' -and $s -match 'same native session')
    ...
    Check RP12_NO_FAKE_EVIDENCE ($k -match 'Never re-consult Thales using\s+fabricated failure, empty evidence' -and $k -match 'unresolved PENDING_OR_INDETERMINATE round cannot')
    ...
    Check RP5_THALES_NO_CHILDREN (Denied $s 'subagent')
    Check RP6_KAEL_MEDIATED ($d -match 'KAEL_MEDIATED' -and $k -match 'direct child')
    Check RP7_SAME_REASONER ($k -match 'SAME native Thales session')
    ...
    Check RP16_MAINTENANCE_OUTSIDE_ROUTING ($k -match 'Kael → Maintenance remains DENIED' -and $k -match 'cannot be TARGET_ROLE' -and $s -match '(?s)Never directly invoke.*?Maintenance' -and $k -match 'Only veyra, orin, kovan, nox, vera, and thales are valid child role IDs')
    ...
    $a=New-Loop
    $a1=Consult $a 'thales-original' 'EVIDENCE_REQUEST'; $a2=Route $a 'probe' 'veyra'
    $a3=WorkerEvent $a 'original-terminal'; $a4=Consult $a 'thales-original' 'ADVICE'
    ...
    $b=New-Loop; $null=Consult $b 'thales-original' 'EVIDENCE_REQUEST'; $null=Route $b 'probe' 'veyra'
    $b1=WorkerEvent $b 'parent-output-missing'; $b2=Route $b 'probe' 'veyra'
    $b3=Consult $b 'thales-original' 'ADVICE'; $b4=WorkerEvent $b 'original-running'
    ...
    $b7=Consult $b 'thales-original' 'ADVICE'; $b.RootFinal=$true
    ...
    $c=New-Loop; $null=Consult $c 'thales-original' 'EVIDENCE_REQUEST'; $null=Route $c 'probe' 'veyra'
    $c1=WorkerEvent $c 'parent-output-missing' $false; $c2=Route $c 'probe' 'veyra'; $c3=Consult $c 'thales-original' 'ADVICE'
    ...
    $n=New-Loop; $null=Consult $n 'thales-original' 'EVIDENCE_REQUEST'; $null=Route $n 'broad' 'veyra'; $null=WorkerEvent $n 'original-terminal'
    $n.Round='REQUESTED'; Check 'CASE_D_NO_PROGRESS' ((Route $n 'broad' 'veyra') -eq 'NO_PROGRESS' -and $n.WorkerCount -eq 1)
    $out=New-Loop; $null=Consult $out 'thales-original' 'EVIDENCE_REQUEST'
    ...
    $manual=Consult $two 'thales-original' 'ADVICE'; $two.RootFinal=$true
    Check 'CASE_B_SECOND_ROUND_EXPLICIT_CONTINUATION_COMPLETE' ($manual -eq 'ADVICE' -and
        ...
        $two.Thales -eq 'thales-original' -and (Complete $two))
