...
function Install([string]$target) {
    $output = (& pwsh -NoProfile -File $installer -Version v0.1.0 -Target $target -SourceRoot $source 2>&1 | Out-String)
...
try {
    ...
    Check 'R1_FRESH_INSTALL' ($first.Code -eq 0 -and $first.Text -match '(?m)^READY\s*$' -and $first.Text -match 'OLYMPUS_INSTALL: v0.1.0 READY_OR_NO_CHANGES')
