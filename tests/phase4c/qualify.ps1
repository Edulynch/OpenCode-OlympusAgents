...
try {
    ...
    Run-Scenario 'M1-M4-M6-STATIC_BOOTSTRAP_ASSERTION' {
        ...
        Assert-Condition ($handoff -match '(?i)maintenance.{0,25}turn.{0,25}finished|maintenance has already finished' -and $handoff -match '(?is)present.*directly' -and $handoff -match '(?i)never\s+merely prepend' -and $handoff -match '(?i)remains running' -and $handoff -match '(?i)running in background|background.{0,80}running') 'Kael natural Maintenance handoff policy missing.' 'BOOTSTRAP_BUG'
