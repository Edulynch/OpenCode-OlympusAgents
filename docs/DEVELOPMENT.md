...
External work ownership is the outer lifecycle rule for work Maintenance launches to fulfill the current user request: PowerShell processes, scripts/controllers, builds, tests, benchmark controllers, disposable validation and OpenCode roots created through session APIs. Normal synchronous commands whose tool call returns only on completion are not detached work. **Foreground/join is the default**, regardless of duration: launch → track → wait/join → collect → validate → final response. Progress updates are not final responses. A launcher exit is not sufficient when subordinate work survives it; reuse the family-aware completion gate below. A reliably completing controller can own its own subordinate jobs; Maintenance waits for and validates its terminal report rather than duplicating that controller's tracking. Independent external jobs can still run in parallel, provided all required jobs are joined before finalization.

Only the user's **explicit background request** allows the Maintenance turn to finish while external work runs. Before launch, establish a durable handoff using the workflow's native PID/session IDs and status/result artifacts (no global registry): report actual identifiers, start time, controller, working directory, available output paths, and exact safe read-only commands for checking status and results. The handoff must state that the chat **will not automatically update** when the job finishes. If durable tracking cannot be supplied, do not launch a silently detached job; report `BACKGROUND_HANDOFF_UNAVAILABLE`. Kael relays an explicit handoff as “Maintenance turn finished; external work running in background”, not “everything complete”. No daemon, scheduler, polling service or notification mechanism is installed.

For local static/synthetic policy and disposable external-process qualification, run `pwsh -NoProfile -File ./tests/external-work-ownership/qualify.ps1`. The live controller cases measure launch, terminal and final times, but do not themselves constitute interactive Maintenance-agent verification; run an interactive `/maintain` smoke separately if required. The harness joins its explicit-background disposable process before it exits, after recording the simulated Maintenance handoff time.
